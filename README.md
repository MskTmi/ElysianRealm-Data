...
- meta：角色关键词元数据
- dist：构建产物与索引文件
- scripts：维护脚本目录，包含 GitHub Actions 自动化工具
...
1. 打开仓库 New Issue，选择关键词投稿模板
2. 确认 `data` 中已存在对应角色图片
3. 填写角色 ID（如 `Human_AstralRing`）
4. 每行填写一个关键词，避免重复与空行
5. 提交后等待 Actions 自动创建 PR
...
1. Fork 并克隆仓库
2. 新建分支（如 `chore/update-keywords-human`）
3. 修改对应 `meta/<id>.json`
4. 如新增角色，需先添加对应图片到 `data`
5. 执行：
...
确保 `dist/elysian-realm-index.json` 已同步更新

6. 提交并推送分支
7. 发起 Pull Request 并说明修改内容
...
仓库可通过 [sync-legacy-data.yml](.github/workflows/sync-legacy-data.yml) 将 master/data 中的最新图片自动同步到 legacy 分支根目录
...
- [MskTmi/Bh3-ElysianRealm-Strategy](https://github.com/MskTim/Bh3-ElysianRealm-Strategy) — 适用于 Mirai 的崩坏3乐往世乐土攻略插件
- [MskTmi/astrbot_plugin_bh3_elysian_realm_strategy](https://github.com/MskTmi/astrbot_plugin_bh3_elysian_realm_strategy) — 适用于 AstrBot 的崩坏3往世乐土攻略插件
- [zipated/Elysian-Realm-plugin](https://github.com/zipated/Elysian-Realm-plugin) — 适用于Yunzai([Miao-Yunzai](https://github.com/yoimiya-kokomi/Miao-Yunzai)、[TRSS-Yunzai](https://github.com/TimeRainStarSky/Yunzai))机器人的崩坏3乐土攻略机器人

## 维护说明
> 图片素材来源于崩坏3通讯中心（月光中心），仅供交流学习使用

> 手动上传图床，在不弃坑的情况下可能会有一到两天延迟，求接力喵

> ~目前的维护需手动[替换文件名](https://blog.msktmi.com/posts/2024/580597065.html)~  
> 当前仓库的 dist 索引可通过 GitHub Actions 自动构建，攻略图的维护可使用[自动化脚本](https://github.com/MskTmi/ImageHostUpdater)  
> 如果您有更好的维护方式，欢迎[讨论](https://github.com/MskTmi/ElysianRealm-Data/discussions/new?category=general)
