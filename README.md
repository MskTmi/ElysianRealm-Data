...
- source：面向维护者，使用 data 与 meta 保存图片和最小元数据，方便维护与自动化
...
### Issue 提交流程（推荐）
> 适合只想补关键词、不想本地改文件的贡献者

1. 打开仓库的 New Issue，选择关键词提交模板
2. 先确认角色图片已存在于 data 目录（文件名需与角色 id 对应）
3. 在角色 ID 中填写准确 id，例如 Human_AstralRing
4. 在关键词列表中每行填写一个关键词，避免空行和重复词
5. 提交 Issue 后等待 Actions 自动处理
6. 机器人会自动创建 PR，审核通过后合并


### PR 贡献流程（手动）
> 适合熟悉 Git 的贡献者，或需要一次性调整多个角色关键词

1. Fork 本仓库并克隆到本地
2. 新建分支，例如 chore/update-keywords-human
3. 编辑对应 meta/<id>.json，仅维护 id 与 keywords 字段
4. 如需新增角色，先确保 data 中已有对应图片文件
5. 本地执行 node scripts/build-index.js，确认 dist/elysian-realm-index.json 已同步更新
6. 提交变更并推送到你的 Fork
7. 发起 Pull Request，说明本次修改的角色与关键词意图
...
- [Bh3-ElysianRealm-Strategy](https://github.com/MskTim/Bh3-ElysianRealm-Strategy) — 基于 Mirai 的QQ机器人插件
- [astrbot_plugin_bh3_elysian_realm_strategy](https://github.com/MskTmi/astrbot_plugin_bh3_elysian_realm_strategy) — 基于 Astrbot 的QQ机器人插件
...
> 当前仓库的 dist 索引可通过 GitHub Actions 自动构建，攻略图的维护可使用[自动化脚本](https://github.com/MskTmi/ImageHostUpdater)，如果您有更好的维护方式，欢迎[讨论](https://github.com/MskTmi/ElysianRealm-Data/discussions/new?category=general)
